...
const INITIAL_TREES = [
    { id: 1001, specie: 'Jacarandá', age: 25, height: 12.5, status: 'Bueno', address: 'Plaza Armenia, Palermo, CABA', lat: -34.5888, lng: -58.4285 },
    ...
    { id: 1006, specie: 'Liquidámbar', age: 18, height: 11.0, status: 'Bueno', address: 'Av. Del Libertador 3200, Palermo, CABA', lat: -34.5768, lng: -58.4063 },
    { id: 1007, specie: 'Jacarandá', age: 35, height: 15.2, status: 'Regular', address: 'Plaza Cortazar, Palermo, CABA', lat: -34.5915, lng: -58.4307 },
    { id: 1008, specie: 'Fresno', age: 8, height: 4.5, status: 'Bueno', address: 'Av. Cabildo 2100, Belgrano, CABA', lat: -34.5630, lng: -58.4568 },
    { id: 1009, specie: 'Tilo', age: 50, height: 18.5, status: 'Malo', address: 'Bolívar 600, San Telmo, CABA', lat: -34.6190, lng: -58.3735 },
    { id: 1010, specie: 'Ceibo', age: 22, height: 8.0, status: 'Regular', address: 'Parque Rivadavia, Caballito, CABA', lat: -34.6185, lng: -58.4358 },
    { id: 1011, specie: 'Liquidámbar', age: 7, height: 4.8, status: 'Bueno', address: 'Juana Manso 1100, Puerto Madero, CABA', lat: -34.6120, lng: -58.3615 },
    { id: 1012, specie: 'Palo Borracho', age: 45, height: 17.5, status: 'Bueno', address: 'Av. 9 de Julio 1200, San Nicolás, CABA', lat: -34.6062, lng: -58.3816 }
];
...
const INITIAL_CLAIMS = [
    {
        ...
        issueType: 'Extracción total',
        description: 'El árbol de Palo Borracho tiene el tronco inclinado y peligro de caída sobre el sendero de peatones.',
        date: '2026-06-01T10:30:00-03:00',
        status: 'En curso',
        notes: 'Inspector evaluó riesgo medio-alto. Se planificó corte preventivo de ramas superiores para el día 12/06/2026.'
    },
    {
        id: 'REC-3012',
        treeId: 1003,
        neighborName: 'María Rodríguez',
        neighborEmail: 'maria.rod@hotmail.com',
        neighborPhone: '11 6666 4321',
        issueType: 'Vereda rota',
        description: 'Las raíces del fresno rompieron completamente las baldosas de la vereda, lo que dificulta el paso de cochecitos.',
        date: '2026-06-03T15:45:00-03:00',
        status: 'Pendiente',
        notes: 'Reclamo recibido. En cola de asignación a cuadrilla de reparación de veredas.'
    },
    {
        id: 'REC-8521',
        treeId: 1009,
        neighborName: 'Carlos Gómez',
        neighborEmail: 'cgomez@outlook.com',
        neighborPhone: '11 4444 8888',
        issueType: 'Árbol enfermo',
        description: 'El tilo está perdiendo las hojas antes de tiempo y tiene una pelusa blanca en las ramas (plaga).',
        date: '2026-06-05T09:15:00-03:00',
        status: 'Resuelto',
        notes: 'Se aplicó tratamiento fitosanitario de pulverización orgánica contra cochinilla algodonosa. El árbol muestra mejoría.'
    }
];

const TREE_IMAGES = {
    'Jacarandá': 'https://images.unsplash.com/photo-1616781297592-fb2721868350?auto=format&fit=crop&w=600&q=80',
    ...
    'Liquidámbar': 'https://images.unsplash.com/photo-1507499739999-097706ad8914?auto=format&fit=crop&w=600&q=80',
...
function navigateTo(viewId) {
    ...
    if (viewId === 'map') {
        ...
        console.log('📍 Map layout visible:', mapLayout?.offsetHeight, 'x', mapLayout?.offsetWidth);
        console.log('📍 Map container visible:', mapContainer?.offsetHeight, 'x', mapContainer?.offsetWidth);
        
        setTimeout(() => {
            console.log('🔄 Iniciando mapa después de render...');
            if (mapContainer?.offsetHeight === 0) {
                console.warn('⚠️ ALERTA: El contenedor del mapa tiene altura 0!');
                // Fuerza una altura mínima
...
function initMainMap() {
    ...
    const mapContainer = document.getElementById('map');
    console.log('Contenedor del mapa:', mapContainer, 'Tamaño:', mapContainer?.offsetWidth, 'x', mapContainer?.offsetHeight);
    ...
    try {
        ...
        // Fuerza recalcular el tamaño del contenedor
        setTimeout(() => {
            STATE.mapInstance.invalidateSize(true);
...
function initClaimCreationMap() {
    ...
    try {
        ...
        console.log('✓ Mapa de crear reclamo inicializado');
    } catch (error) {
...
function handleClaimMapClick(e) {
    ...
    }).addTo(STATE.claimCreationMap).bindPopup('Ubicación seleccionada');
...
function showTreeDetails(tree) {
    ...
    document.getElementById('tree-detail-edad').textContent = `${tree.age} años`;
...
function searchAddress() {
    ...
    if (matchedTree) {
        ...
        alert('Dirección o especie no encontrada en el censo. Intenta buscar "Palermo", "Belgrano" o una especie como "Jacarandá".');
    }
}
...
function submitClaimForm(e) {
    ...
    const newClaim = {
        ...
        status: 'Pendiente',
        notes: 'Reclamo ingresado. Se programará inspección técnica de campo en breve.'
    };
...
function searchClaimTracking() {
    ...
    // Populate notes
    document.getElementById('track-result-notes').textContent = claim.notes || 'No hay comentarios del inspector aún.';
...
function renderCensusExcelTable() {
    ...
    if (trees.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" style="text-align: center; color: var(--color-text-gray);">No se encontraron árboles en el censo.</td></tr>';
        return;
    }
...
function openTreeModal(tree = null) {
    ...
    if (tree) {
        title.textContent = `Editar Árbol #${tree.id}`;
        ...
        title.textContent = 'Agregar Nuevo Árbol al Censo';
...
function deleteTree(treeId) {
    if (!confirm(`¿Estás seguro de que deseas eliminar el árbol #${treeId} del censo general?`)) return;
...
function exportToCSV() {
    ...
    csvContent += 'ID;Especie;Edad (Años);Altura (m);Estado Fitosanitario;Dirección;Latitud;Longitud\r\n';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof L === 'undefined') {
        console.error('Leaflet no está cargado. Verifica que la librería esté disponible.');
    } else {
...
document.getElementById('form-public-register-action').addEventListener('submit', (e) => {
    ...
    alert('¡Registro exitoso! Bienvenido ' + name + '. Accediendo al sistema...');
    STATE.currentUser = 'public';
    navigateTo('map');
});
...
document.getElementById('btn-make-claim').addEventListener('click', () => {
    ...
    if (!STATE.currentUser) {
        alert('Debes iniciar sesión para hacer un reclamo. Serás redirigido al login.');
        navigateTo('login');
        return;
    }
