...
window.loadTreesList = function() {
    ...
    window.trees.forEach(t => {
        ...
        card.className = `list-item-card ${window.selectedTreeId === t.id ? 'active' : ''}`;
        card.onclick = () => window.selectTree(t.id);
...
window.selectTree = function(id) {
    window.selectedTreeId = id;
...
window.filterTrees = function() {
    ...
    const filtered = window.trees.filter(t => {
...
window.loadTreesFromServer = async function() {
    try {
