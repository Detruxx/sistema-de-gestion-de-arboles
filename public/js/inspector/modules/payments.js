// --- Lógica del Módulo de Pagos e Inspector ---

window.workOrders = [];
window.selectedWorkOrderId = null;
...
window.loadPaymentsList = function () {
    ...
    const completedWorks = window.workOrders.filter(w => w.work_status === 'Finalizado');
    ...
    completedWorks.forEach(w => {
        ...
        card.className = `list-item-card ${window.selectedWorkOrderId === w.id ? 'active' : ''}`;
        card.onclick = () => window.selectWorkOrderPayment(w.id);
...
window.selectWorkOrderPayment = function (id) {
    window.selectedWorkOrderId = id;
    window.loadPaymentsList();

    const w = window.workOrders.find(item => item.id === id);
...
window.togglePaymentStatus = async function (id) {
    ...
    try {
        ...
        window.selectWorkOrderPayment(id);
        window.loadPaymentsList();
...
window.loadWorkOrdersFromServer = async function () {
    try {
...
document.addEventListener('DOMContentLoaded', () => {
    if (window.currentUserRole === 'inspector') {
        window.loadWorkOrdersFromServer();
    }
});
